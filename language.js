...
(()=>{
 ...
 function update(){observer.disconnect();if(document.documentElement.lang!==lang)document.documentElement.lang=lang;label.querySelector('span').textContent=lang==='en'?'Language':lang==='ja'?'言語':'語言';select.setAttribute('aria-label',lang==='en'?'Select language':lang==='ja'?'言語を選択':'選擇語言');const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);let node;while(node=walker.nextNode()){if(node.parentElement.closest('script,style,.language-picker'))continue;const next=translate(node.nodeValue);if(next!==node.nodeValue)node.nodeValue=next}
